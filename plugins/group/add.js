...
let handler = async (m, { conn, text, participants }) => {
	...
	if (who.length == 0) throw `Who do you want to add?`
	...
	if (users.length == 0) throw `@${not.join(', @')} not a WhatsApp user`
	if (not.length > 0) m.reply(`@${not.join(', @')} not a WhatsApp user`)
	let img = await conn.profilePictureUrl(m.chat, 'image').catch(_ => 'https://imgur.com/WnKjrJt')
	for (let i of users) {
