...
} = (await import('@shizodevs/baileys')).default
...
/** @type {import('@shizodevs/baileys').UserFacingSocketConfig} */
 ...
 *  credsUpdate?: () => void 
 * }} EventHandlers
 * @typedef {Required<import('@shizodevs/baileys').UserFacingSocketConfig>['logger']} Logger
...
async function start(oldSocket = null, opts = { store, logger, authState }) {
	...
	if(usePairingCode && !conn.authState.creds.registered) {
		...
		let phoneNumber = ''
		do {
			phoneNumber = await question(chalk.blueBright('ENTER A VALID NUMBER START WITH REGION CODE. Example : 9178499xxx:\n'))
 ...
 * @this {Socket}
 * @param {StartOptions} opts
 * @param {import('@shizodevs/baileys').BaileysEventMap<unknown>['connection.update']} update
 */
